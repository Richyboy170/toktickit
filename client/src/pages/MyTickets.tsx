  ...
  const load = useCallback(async () => {
    ...
    try {
      const [tickets, nextCategories, nextSystems] = await Promise.all([
        listTickets(user ? undefined : requester.id, toParams(applied, page)),
        getCategories(),
        getRelatedSystems(),
      ]);
      setResult(tickets);
  ...
  return (
    <section className="page-card" aria-labelledby="my-tickets-title">
      ...
      {state === "ready" && result && result.items.length > 0 && <>
        ...
        <div className="ticket-table-wrap"><table className="ticket-table"><thead><tr><th scope="col">Ticket</th><th scope="col">Summary</th><th scope="col">Category / System</th><th scope="col">Priority</th><th scope="col">Status</th><th scope="col">Updated</th><th scope="col">Action</th></tr></thead><tbody>{result.items.map((ticket) => <tr key={ticket.id}><td><Link to={`/tickets/${ticket.id}`}>{ticket.ticketNumber}</Link><small>{displayDate(ticket.ticketDate)}</small></td><td>{ticket.summary}</td><td>{ticket.category.name}<small>{ticket.relatedSystem.name}</small></td><td><span className={`badge badge--${ticket.requestedPriority.toLowerCase()}`}>{label(ticket.requestedPriority)}</span></td><td><span className="badge badge--status">{label(ticket.currentStatus)}</span></td><td>{displayDate(ticket.updatedAt)}</td><td><Link to={`/tickets/${ticket.id}`}>View Ticket</Link></td></tr>)}</tbody></table></div>
        <div className="ticket-cards">{result.items.map((ticket) => <article className="ticket-card" key={ticket.id}><div><Link className="ticket-card__number" to={`/tickets/${ticket.id}`}>{ticket.ticketNumber}</Link><span className="badge badge--status">{label(ticket.currentStatus)}</span></div><h2>{ticket.summary}</h2><dl><div><dt>Category</dt><dd>{ticket.category.name}</dd></div><div><dt>System</dt><dd>{ticket.relatedSystem.name}</dd></div><div><dt>Priority</dt><dd>{label(ticket.requestedPriority)}</dd></div><div><dt>Updated</dt><dd>{displayDate(ticket.updatedAt)}</dd></div></dl><Link className="button button--secondary button-link ticket-card__action" to={`/tickets/${ticket.id}`}>View Ticket</Link></article>)}</div>
